...
class Agent(BaseAgent):
    ...
    def init_config(self, config_data):
        ...
        for hero_id in my_heroes:
            skill_id = self._select_summoner_skill(hero_id, opponent_heroes)
            select_skills[hero_id] = skill_id
        return select_skills

    def _select_summoner_skill(self, hero_id, opponent_heroes):
        # Deterministic matchup rules reduce evaluation variance.
        # 确定性对位规则降低评估波动。
        opponent_hero = opponent_heroes[0] if opponent_heroes else None
        if hero_id == 112:
            return 80115  # 鲁班七号优先闪现，补足生存和拉扯能力
        if hero_id == 133:
            if opponent_hero == 133:
                return 80107  # 狄仁杰内战优先净化，降低控制链风险
            return 80108  # 狄仁杰对鲁班优先终结，强化斩杀窗口
        return 80115
